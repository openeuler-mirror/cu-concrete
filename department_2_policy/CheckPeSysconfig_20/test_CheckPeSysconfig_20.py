...
def prepare_files():
    ...
    yield
    for fp in [pkl_path, '/tmp/CheckPeSysconfig_20.yaml', file_path, backup_path]:
...
def build_instance():
    mod = load_module()
    cls = getattr(mod, 'CheckPeSysconfig_20')
    obj = cls()
    obj.config_file = '/tmp/CheckPeSysconfig_20.yaml'
    ...
    if os.path.exists(yaml_path):
        ...
        obj.config = {
            ...
            'description': '确保 sysconfig 相关文件权限为 644'
        }
    # some modules use a 'cahnge' typo elsewhere; set it defensively
...
def test_finalfix():
    ...
    assert status_df.loc['220', 'status'] == 2
...
def test_rollback_updates_status_when_check_fails(monkeypatch):
    ...
    class FakeBSF:
        ...
        def file_permission(*args):
            ...
            if len(args) == 1:
                # return a different mode than obj.config['change']['value'] so check() fails
                return ('755', 0)
            return None

    monkeypatch.setattr(mod, 'bsf', FakeBSF)

    obj.status_form.loc['220', 'status'] = 1
    obj.status_form.to_pickle(pkl_path)

    obj.rollback()
    status_df = pd.read_pickle(pkl_path)
    assert status_df.loc['220', 'status'] == 0
