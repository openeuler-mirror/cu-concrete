...
class AuditDockerd_1(base_fix):

    def __init__(self):
        super().__init__()
        self.current_dir = os.path.dirname(os.path.abspath(__file__))
        self.config_file = os.path.join(self.current_dir, 'AuditDockerd_1.yaml')
    ...
    def fix(self):
        ...
        with open(self.config['query']['path'][1], 'w') as f:
            f.write(self.config['query']['form'] + '\n')
        bsf.delete_audit_rule()
        re = bsf.reload_audit_rules()
