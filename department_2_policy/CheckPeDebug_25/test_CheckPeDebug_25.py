...
def load_module():
    ...
    module_path = os.path.join(base_dir, 'CheckPeDebug_25.py')
    spec = importlib.util.spec_from_file_location('CheckPeDebug_25_module', module_path)
    mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod)
    return mod

yaml_path = os.path.join(os.path.dirname(__file__), 'CheckPeDebug_25.yaml')
pkl_path = '/tmp/test_data_status_checkpedebug.pkl'
file_path = '/tmp/test_pedebug_path'
backup_path = '/tmp/test_checkpedebug_bak'

@pytest.fixture(autouse=True)
def prepare_files():
    if os.path.exists(yaml_path):
        os.system(f'cp {yaml_path} /tmp/CheckPeDebug_25.yaml')
    ...
    yield
    for fp in [pkl_path, '/tmp/CheckPeDebug_25.yaml', file_path, backup_path]:
...
def build_instance():
    mod = load_module()
    cls = getattr(mod, 'CheckPeDebug_25')
    obj = cls()
    obj.config_file = '/tmp/CheckPeDebug_25.yaml'
    ...
    if os.path.exists(yaml_path):
        ...
        obj.config = {
            ...
            'description': '确保 debug 文件权限为 660'
        }
...
def test_finalfix():
    ...
    assert status_df.loc['225', 'status'] == 2
...
def test_check_true_on_permission_error_code(monkeypatch):
    mod, obj = build_instance()
    # simulate file_permission failure (non-zero return code); code should keep except_value=True
...
def test_rollback_updates_status_when_check_fails(monkeypatch):
    ...
    obj.rollback()
    status_df = pd.read_pickle(pkl_path)
    assert status_df.loc['225', 'status'] == 0
