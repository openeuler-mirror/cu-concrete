...
def load_module():
    ...
    module_path = os.path.join(base_dir, 'AuditConToml_10.py')
    spec = importlib.util.spec_from_file_location('AuditConToml_10_module', module_path)
    mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod)
    return mod
yaml_path = os.path.join(os.path.dirname(__file__), 'AuditConToml_10.yaml')
pkl_path = '/tmp/test_data_status_auditcontoml.pkl'
service_name = 'con_toml'
file_service = '/tmp/test_con_toml_service'
rule_file = '/tmp/test_10-con.toml'
auditctl = '/tmp/test_auditctl'
backup_path = '/tmp/test_con_toml_bak'

@pytest.fixture(autouse=True)
def prepare_files():
    if os.path.exists(yaml_path):
        os.system(f'cp {yaml_path} /tmp/AuditConToml_10.yaml')
    ...
    yield
    for fp in [pkl_path, '/tmp/AuditConToml_10.yaml', file_service, rule_file, auditctl, backup_path]:
        if os.path.exists(fp):
            os.remove(fp)

def build_instance():
    mod = load_module()
    cls = getattr(mod, 'AuditConToml_10')
    obj = cls()
    obj.config_file = '/tmp/AuditConToml_10.yaml'
    ...
    if os.path.exists(yaml_path):
        ...
        obj.config = {'dep': 2, 'id': 10, 'query': {'path': [file_service, rule_file, auditctl], 'form': f'[con.toml] -p wa -k contoml'}, 'change': {'set': 'auditctl', 'value': "'^[^#;]'"}, 'backup_path': backup_path, 'description': '确保 con.toml 配置文件相关审计规则存在'}
...
def test_finalfix():
    ...
    assert status_df.loc['210', 'status'] == 2
...
def test_rollback_updates_status_when_check_fails(monkeypatch):
    ...
    class FakeBSF:
        ...
        def pipe_grep_shell(form_arg, path, value):
            return ('', 1)
    monkeypatch.setattr(mod, 'bsf', FakeBSF)
    obj.status_form.loc['210', 'status'] = 1
    obj.status_form.to_pickle(pkl_path)
    obj.rollback()
    status_df = pd.read_pickle(pkl_path)
    assert status_df.loc['210', 'status'] == 0
