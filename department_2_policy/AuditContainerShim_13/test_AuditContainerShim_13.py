...
def build_instance():
    ...
    if os.path.exists(yaml_path):
        ...
        obj.config = {'dep': 2, 'id': 13, 'query': {'path': [file_service, rule_file, auditctl], 'form': f'-w /usr/bin/{service_name} -p wa -k containershim'}, 'change': {'set': 'auditctl', 'value': "'^[^#;]'"}, 'backup_path': backup_path, 'description': '确保 container shim 相关审计规则存在'}
...
def test_rollback_updates_status_when_check_fails(monkeypatch):
    ...
    class FakeBSF:
        ...
        def pipe_grep_shell(form_arg, path, value):
            pass
    monkeypatch.setattr(mod, 'bsf', FakeBSF)
    obj.status_form.loc['213', 'status'] = 1
    obj.status_form.to_pickle(pkl_path)
    obj.rollback()
    status_df = pd.read_pickle(pkl_path)
    assert status_df.loc['213', 'status'] == 0
