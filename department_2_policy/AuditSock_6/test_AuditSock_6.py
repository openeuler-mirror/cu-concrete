...
def load_module():
    ...
    module_path = os.path.join(base_dir, 'AuditSock_6.py')
    spec = importlib.util.spec_from_file_location('AuditSock_6_module', module_path)
    mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod)
    return mod

yaml_path = os.path.join(os.path.dirname(__file__), 'AuditSock_6.yaml')
pkl_path = '/tmp/test_data_status_auditsock.pkl'
file_sock = '/tmp/test_sock_file'
rule_file = '/tmp/test_06-sock.rules'
auditctl = '/tmp/test_auditctl'
backup_path = '/tmp/test_sock_bak'

@pytest.fixture(autouse=True)
def prepare_files():
    if os.path.exists(yaml_path):
        os.system(f'cp {yaml_path} /tmp/AuditSock_6.yaml')
    ...
    yield
    for fp in [pkl_path, '/tmp/AuditSock_6.yaml', file_sock, rule_file, auditctl, backup_path]:
...
def build_instance():
    mod = load_module()
    # class in source file is AuditService_5 per current content
    cls = getattr(mod, 'AuditSock_6')
    obj = cls()
    ...
    if os.path.exists(yaml_path):
        ...
        obj.config = {
            ...
            'id': 6,
            'query': {
                'path': [file_sock, rule_file],
                'form': '-w /var/run -k sock'
            },
            'change': {
                'set': 'auditctl',
                'value': "'^[^#;]'"
...
def test_finalfix():
    ...
    assert status_df.loc['26', 'status'] == 2
...
def test_rollback_updates_status_when_check_fails(monkeypatch):
    ...
    obj.status_form.loc['26', 'status'] = 1
    obj.status_form.to_pickle(pkl_path)

    obj.rollback()
    status_df = pd.read_pickle(pkl_path)
    assert status_df.loc['26', 'status'] == 0
