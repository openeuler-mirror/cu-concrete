...
class LockFile_29(base_fix):    
    def __init__(self):
        ...
        self.config_file = os.path.join(self.current_dir, "LockFile_29.yaml")
    ...
    def fix(self):
        ...
        cmd=['chattr',self.config['change']['value'][0],self.config['query']['path'][0]]
        base_shell(cmd)

        cmd=['chattr',self.config['change']['value'][0],self.config['query']['path'][1]]
        base_shell(cmd)
    ...
    def rollback(self):
        ...
        cmd=['chattr',self.config['change']['value'][1],self.config['query']['path'][1]]
        base_shell(cmd)

        cmd=['chattr',self.config['change']['value'][1],self.config['query']['path'][2]]
        base_shell(cmd)
        result = self.check()
