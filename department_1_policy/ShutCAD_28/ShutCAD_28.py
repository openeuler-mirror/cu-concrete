...
# 优化：统一日志变量命名
class ShutCAD_28(base_fix):    
    def __init__(self):
        super().__init__()
        self.current_dir = os.path.dirname(os.path.abspath(__file__))
        self.config_file = os.path.join(self.current_dir, "ShutCAD_28.yaml")
    ...
    def fix(self):
        ...
        if os.path.exists('/etc/systemd/system/ctrl-alt-del.target'):
            cmd=['rm','/etc/systemd/system/ctrl-alt-del.target']
            base_shell(cmd)
    ...
    def rollback(self):
        ...
        if os.path.exists(self.pkl_file):
            ...
            self.status_form = pd.DataFrame(columns=['status', 'module_name', 'module_path'])
        cmd = ['systemctl', self.config['change']['value'][1], self.config['query']['path']]
        base_shell(cmd)
        result=self.check()
        if result==False:
