...
# 优化：统一日志变量命名
class ClearHistoryCmd_25(base_fix):    
    def __init__(self):
        super().__init__()

        self.current_dir = os.path.dirname(os.path.abspath(__file__))
        self.config_file = os.path.join(self.current_dir, "ClearHistoryCmd_25.yaml")
    ...
    def fix(self):
        ...
        self.status_form.to_pickle(self.pkl_file)
        bsf.cp_shell(self.config['query']['path'],self.config['query']['path1'])
        cmd=['rm',self.config['query']['path']]
        base_shell(cmd)
