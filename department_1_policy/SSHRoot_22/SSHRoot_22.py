...
# 优化：统一日志变量命名
class SSHRoot_22(base_fix):    
    def __init__(self):
        super().__init__()
        self.current_dir = os.path.dirname(os.path.abspath(__file__))
        self.config_file = os.path.join(self.current_dir, "SSHRoot_22.yaml")
    ...
    def fix(self):
        ...
        # result2=bsf.grep_shell(self.config['query']['form'][1],self.config['query']['path'][1])
        # bsf.sed_shell(result2[0],self.config['change']['value'][1],self.config['query']['path'][1])
    ...
    def rollback(self):
        ...
        cmd1 = self.config['change']['set'][0].split(" ")
        cmd2 = self.config['change']['set'][2].split(" ")
        base_shell(cmd1)
        base_shell(cmd2)
        cmd3=['systemctl','restart','sshd']
        base_shell(cmd3)
