...
def load_check_class(department_id):
    ...
    listfolder = [
        folder for folder in os.listdir(department_path)
        if folder not in ['.git', '__pycache__'] 
        and os.path.isdir(os.path.join(department_path, folder))  
    ]
    
    #读取pkl，获取加固项状态 0：未加固，1：加固失败，2：加固成功
...
def load_departments(department_ids):
    ...
    # 收集所有要处理的模块条目 (module_name, module_path, department_data_pkl)
    entries = []
    ...
    # 为每个 dept 准备 status DataFrame 缓存
    status_cache = {}
